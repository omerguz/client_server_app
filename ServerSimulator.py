...
class Server:
    ...
    def send_message_to_clients(self, player, msg):
        """
        ...
        try:
            player.clientSocket.send(msg.encode("utf-8"))
            return True
            # updated_players_data.append(player_data)
        except Exception as e:
            print(f"Error sending message to {player.playerName}: {e}. Need to remove from the list.")
            return False
            ...
            try:
                ...
                while len(current_players) >= 1:
                    ...
                    for player in current_players:
                        self.send_message_to_clients(player, msg)
                    msg = ""
